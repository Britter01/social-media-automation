...
class QualityAgent:
    ...
    _IMAGE_SYSTEM = (
        ...
        "BRAND WATERMARK — ALWAYS PERMITTED, NEVER FLAG:\n"
        "Every image will have the Brite Tech Lifestyle brand watermark composited into "
        "one corner. This watermark shows the word 'Brite' (sometimes with 'TECH LIFESTYLE' "
        "as a subtitle beneath it) in a corner of the image. This is intentional, required, "
        "and must NEVER be flagged regardless of its size, position, or how prominent it "
        "appears. It is not a violation under any standard below.\n\n"
        "Check the image against these standards:\n"
        "  1. The Brite Tech Lifestyle brand watermark must be visible in one corner.\n"
        "  2. There must be NO solid-colour header or footer bar overlaid on the photo "
        "(e.g. a dark bar with a title or category label baked into the image by the AI).\n"
        "  3. There must be no large AI-generated or AI-hallucinated text overlay anywhere "
        "in the image — this does NOT include the Brite Tech Lifestyle brand watermark.\n"
        "  4. Any text hallucinated INTO the photo itself (words on walls, floating text, "
        "unrelated brand names on screens) is a failure.\n\n"
        "IMPORTANT: 'Brite', 'BRITE', 'TECH LIFESTYLE', or any combination of these words "
        "appearing in a corner of the image is the brand watermark — it is correct and must "
        "pass QC without question.\n\n"
